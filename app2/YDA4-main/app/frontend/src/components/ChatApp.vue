<template>
  <div class="chat-container">
    ...
    <!-- Chat ready state (for logged-in users) -->
    <div class="initial-state" v-if="!hasTranscription && isAuthenticated && messages.length === 0">
      <div class="welcome-card">
        <h3>Ready to Chat</h3>
        <p>You can start chatting directly with the AI, or add a podcast or YouTube URL from the sidebar to chat about specific content.</p>
      </div>
    </div>

    <!-- Chat Section (shows when authenticated) -->
    <div v-if="isAuthenticated" class="chat-section">
      ...
      <!-- Transcription Summary (only show if we have transcription) -->
      ...
      <!-- Reset Button (only show if we have transcription) -->
      <div v-if="hasTranscription" class="reset-section">
        <button @click="reset" class="reset-btn">Process New Content</button>
      </div>
    </div>
  </div>
</template>
...
const sendMessage = async () => {
  ...
  try {
    const response = await fetchWithAuth(`${API_BASE_URL}/api/chat/stream`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        message: txt,
        context: transcription.value || null,
        model: selectedModel.value
      })
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error?.message || errorData.detail || 'Failed to get chat response');
    }
        ...
        if (line.startsWith('data: ')) {
          try {
            ...
            if (data.response) {
              ...
              messages.value[placeholderIndex].text = accumulatedText;
              scrollToBottom();
            }
    ...
    messages.value[placeholderIndex].text = errorMessage;
  } finally {
