...
running in a container or locally.

The QdrantDB class:
  - Initializes a Qdrant client.
  - Ensures the target collection exists (creating it if it doesn't). 1 collection per USER.
...
Assumptions:
  - Qdrant is running locally or in a container.
...
class QdrantDB:
    def __init__(self, collection_name: str, embedding_model: Embeddings, vector_size: int = 384, client_url: str = None):
        """
        ...
        if client_url is None:
            client_url = os.getenv('QDRANT_URL', 'http://localhost:6333')
